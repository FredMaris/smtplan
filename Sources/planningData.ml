...
(* OP iter *)  
  method operator_iter = domain#operator_iter
  ...
  *)
  ...
  method private search_fluent (atom : Atom.t) =
  ...
  method private search_fluenti (atom : Atom.t) =
	  ...
	  (self#create_action o#name params (calculate_duration params o#duration) o#quality
		...
		(fun atom -> begin append_timedata atom params; ((
		   let (fluent, present) = self#search_fluenti (atom#instantiate params) in
		     ...
		     (*Utils.print "%s instantiated as %s\n" atom#to_string fluent#to_string; flush stderr;*)
		     fluent
...
(*(Array.map (fun (c,e) -> ((self#create_fluent c,c#timedata),(self#create_fluent e,e#timedata))) o#condadd))*)
         (Array.map (fun (atomc,atome) ->
	     	begin
	     (*	Utils.print "Treating 'when' effect:\n"; *)
	     		append_timedata atomc params;
	     		append_timedata atome params;
	     	(*	Utils.print "%s->%s" atomc#to_string atome#to_string; flush stderr; *)
	     		let atomci = atomc#instantiate params in
	     		let atomei = atome#instantiate params in
	     	(*	Utils.print " instantiated as %s->%s\n" atomci#to_string atomei#to_string; flush stderr; *)
	     		let fluentc = fst (self#search_fluent atomci) in
	     		let fluente = fst (self#search_fluent atomei) in
				let result = ((fluentc,atomc#timedata),(fluente,atome#timedata)) in
	     	(*	let result = ( ((fst (self#search_fluent (atomc#instantiate params))),atomc#timedata) , ((fst (self#search_fluent (atome#instantiate params))),atome#timedata) )
	     		in *)
			(*	Utils.print "Done\n"; flush stderr; *)
				result
	     	end
  ...
  *)
  ...
  method private final_representation =
	...
	Array.iter (fun (_,f) -> f#add_condproducer action) action#condadd ; 
(* Array.iter (fun (c,e) -> Utils.print "condeff: %s %s\n" c#to_string e#to_string) action#condadd ; *)
	true
      with Exit -> false
    in
...
(*** POUR T2L-SMTPLAN2015 ***)
(*	domain#operator_iter (fun op -> Utils.print "%s\n" op#name;
