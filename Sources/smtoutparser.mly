...
%start model
%type <(string * float) list> model
%%


model:
| LP list_of_var_values { $2 }

list_of_var_values:
| RP { [] }
| var_value list_of_var_values { $1 :: $2 }

var_value:
| LP IDENT number RP { ($2,$3)}
| LP IDENT LP TYPE number RP RP { ($2, -.$5)}
| LP IDENT LP DIVIDE number number RP RP { ($2,$5 /. $6) }
| LP IDENT LP DIVIDE LP TYPE number RP number RP RP { ($2,-.$7 /. $9) }
